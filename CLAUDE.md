...
- **`/live/` symlinks point at the date tree** — letting `cd ~/views/fireflies-meetings/live/<id>` land in the right `YYYY-MM/DD/<slug>/` dir. Don't replace the symlinks with real directory entries; the symlink is the contract.
